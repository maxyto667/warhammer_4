...
export default function CharacterManager() {
  ...
  return (
    <>
      <header className="parchment-panel rounded-lg border-2 border-imperial-gold/50 p-3.5 mb-6 shadow-grim flex flex-col md:flex-row items-center justify-between gap-4 no-print">
            ...
            <div className="flex items-center gap-2 text-[11px] text-parchment-400">
              ...
              {isSaving ? (
                <span className="text-amber-400 flex items-center gap-1 animate-pulse">
                  <RefreshCw size={10} className="animate-spin" /> Guardando local...
                </span>
              ) : (
                <span className="text-emerald-400 flex items-center gap-1">
                  <CheckCircle2 size={11} /> Guardado local OK
                </span>
              )}

              {/* Indicador de Sala Cloud */}
              {roomCode && (
                <>
                  <span>•</span>
        ...
        <div className="flex flex-wrap items-center gap-2 justify-end">
          ...
          <button
            ...
            className={`px-3 py-1 rounded text-xs flex items-center gap-1.5 font-subheading font-bold transition-all shadow-sm ${
              roomCode 
                ? 'bg-amber-950/80 border border-amber-500/60 text-amber-200 hover:bg-amber-900/90' 
                : 'bg-grim-950 hover:bg-grim-900 text-imperial-gold border border-imperial-gold/40 hover:border-imperial-gold'
            }`}
            title="Gestionar sala en la nube, código para compartir y sincronización online"
          >
            <Cloud size={14} className={roomCode ? "text-emerald-400" : "text-amber-400"} />
            {roomCode ? `Sala: ${roomCode}` : '☁️ Nube / Sala'}
          </button>
      ...
      {/* Modal de persistencia en la nube por Códigos de Sala */}
      <CloudRoomModal 
